...
OmegaMais  = (Omega_1 + Omega_2)/2
OmegaMenos = (Omega_1 - Omega_2)/2

m = 1
k = 1


V1(t)=(k*(A*(cos((Omega_1-Omega_2)/2 *t) * cos((Omega_1 + Omega_2)/2 * t)))**2)/2
V2(t)=(k*(A*(sin((Omega_1-Omega_2)/2 *t) * sin((Omega_1 + Omega_2)/2 * t)))**2)/2
...
T1(x) = 0.5*m*(dotx1(x))**2
T2(x) = 0.5*m*(dotx2(x))**2
E1(x) = T1(x)+V1(x)
E2(x) = T2(x)+V2(x)

plot [0:60] V1(x) + V2(x) w l ls 1
set output "5o1.png"
plot [0:60] T1(x) + T2(x) w l ls 2
set output "5o2.png"
plot [0:60] E1(x)+E2(x) w l ls 3
